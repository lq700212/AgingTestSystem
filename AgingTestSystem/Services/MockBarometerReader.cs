    ...
    public class MockBarometerReader : IBarometerReader
    {
        ...
        public BarometerData ReadData(int deviceId)
        {
            ...
            lock (_randomLock)
            {
                ...
                // - 85% 概率生成"真空良好"：-96 ~ -100 kPa（低于报警阈值 -95，不报警）
                //   这样点"启动运行"后真空能正常建立，演示"测试中→老化计时→自动停止"的完整流程
                // - 15% 概率生成"真空较差"：-1 ~ -90 kPa（高于阈值，触发报警联动演示）
                if (_random.Next(100) < 85)
                {
                    pressureInt = -_random.Next(96, 101);     // 真空良好（低于 -95 阈值）
                }
                else
                {
                    pressureInt = -_random.Next(1, 91);       // 真空较差（高于阈值，会报警）
                }
