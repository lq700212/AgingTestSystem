...
namespace AgingTestSystem.Views
{
    ...
    /// └──────────────────────────────────────────────┘
    /// 标注说明：
    /// - 行1：上电/下电块(57,29,52,23) + 工作状态块(153,29,52,23) + 选中框(212,4,23,23)
    /// - 行2：真空压力值框(57,67,93,21) + 真空开/关块(153,67,52,21)
    /// - 行3：SN 值框(57,93,148,21)、配方值框(57,118,148,21)
    /// - 行4：延时开启值框(57,147,80,21) + 设置按钮(145,145,60,50)、延时到达值框(57,172,80,21)
    /// - 标签列：真空压力(3,70)/SN:(3,96)/配方:(3,121)/延时开启(3,150)/延时到达(3,175)
    /// - 值框文字左内边距：ValueTextLeftPadding=6px（V1.52，文字不贴值框左边框，值框坐标不变）
    /// - 状态块配色见下方"状态块配色"；颜色值均可由 PanelLayout.json 覆盖
    /// - 【V1.58.6 对齐】延时开启/延时到达两行整体下移 4px，使两行中心(157.5+182.5)/2=170
    ///   与右侧"设置"按钮中心(145+25=170)垂直居中对齐，视觉上两行不再"偏上"。
