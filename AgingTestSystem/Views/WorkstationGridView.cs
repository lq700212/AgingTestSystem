...
namespace AgingTestSystem.Views
{
    ...
    /// │ × 9行（行高152，每格内容144+上下缝8）       │ 选中→[取消]│
    /// │ （竖排：[取]/[消]上下两格）                  │          │
    /// └───────────────────────────────────────────┴──────────┘
    /// （保持 8×9=72；内容总宽 8×209＋行全选列 48＝1720，总高 9×152＝1368）
    /// 行全选按钮高 = 面板内容高-1(=143)，含边框后上下边缘与工作站显示框(144)完全对齐；
    ...
    /// 二、单个面板内容（204×144，坐标均为"相对面板左上角"；紧凑布局：
    /// 标题/选中框并入第一行与上下电/真空同行，纵向省 42px 换缩放比；
    /// 下电 54→49→45（左缘对齐值框列 74）、真空 54 不动、值框 130×18→121×16（配方 121×18）、
    /// 上电/真空块高 18→20（V1.96：19px 字在 18 框里挤）、
    /// 延时框 66→72→74、时间字 9→9.5pt、设置按钮 50×42→46×36→42×36（字 12→11pt）、
    /// 选中框 16→14→18（对勾保留）；正文 9→10pt、
    /// 标题 12→11pt，物理字号反而 4.7→7.0pt 更清；标签列 56→65 装四字长标签）：
    /// ┌──────────────────────────────────────────────┐
    /// │ NO.1 ┌──────┐ ┌────────┐        ┌──────┐      │ ← 第一行同行四件套
    /// │ (6,6)│上电/下电│ │真空开/关│      │选中框✓ │  │    编号11pt＋下电45×20＋真空54×20＋选中框18×18
    /// │      └──────┘ └────────┘        └──────┘      │
    /// │ 真空压力 ┌──────────────────────────────┐    │
    /// │          │  78 kPa                      │    │
    /// │          └──────────────────────────────┘    │
    ...
    /// └──────────────────────────────────────────────┘
    /// 标注说明（括号内为锚定关系）：
    /// - 行1（Y=4,H=20）：编号 NO.1(6,6；LeftMargin=6+TopMargin=6) + 上电/下电块(74,4,45,20；
    ///   左缘对齐 SN 框＋右缘贴真空关左缘 Gap=4，宽两端推导＋Y/H 对齐真空块） + 真空开/关块(123,4,54,20；
    ///   右缘贴选中框左缘 Gap=4＋TopMargin=4） + 选中框(181,4,18,18；RightMargin=5+TopMargin=4，
    ///   边长取缩放后较小边恒正方形，见 SelectBoxSide）
    /// - 行2：压力值框(74,30,121,16；左缘对齐 SN 框＋右缘对齐设置按钮，宽 121；
    ///   Y 吊真空块下方 TopToBottomGap=6，Y=4+20+6=30)
    ///   + 电流值框 RcCurrentValue(74,50,121,16；与压力同界；
    ///   Y 吊压力框下方 Gap=4；ShowCurrentRow 关=整行不画；开=面板 144→160、SN 54→70、
    ///   配方 79→95、延时 102/120→118/136、按钮 101→117，间距全都不变，见下方"V1.77 开态几何")
    /// - 行3：SN 值框(74,54,121,16；吊电流行下方 TopToBottomGap=4：
    ///   关电流电流行高按 0，Y=50+0+4=54；开时 Y=50+16+4=70；SN 序列号最长，值框保持最宽一档)
    /// - 行4：配方值框(74,79,121,18；下缘贴设置按钮上缘、Gap=4)
    /// - 行5：延时时间值框(74,102,74,16；框内值走 9.5pt 时间字，标签仍 10pt；
    ///   以设置按钮中心为基准、CenterOffsetY=-9) +
    ///   设置按钮(153,101,42,36；下缘距面板底 BottomMargin=7) + 烧屏时间值框(74,120,74,16；CenterOffsetY=9)
    /// - 编号：NO.1(6,6)（LeftMargin=6 + TopMargin=6，与第一行同行）
    /// - 标签列：真空压力(9,30)/SN:(9,54)/配方:(9,80)/延时时间(9,102)/烧屏时间(9,120)
    ///   （X=9 为右缘贴合压力框左缘推导 74-65=9；Y 以各自框中心为基准、VerticalCenterOffset=-1；
    ///   标签列宽 65：四字 10pt 实测 65px 零余量装得下，"00:00:00" 9.5pt 实测约 63px 装进 74 框）
    ...
    ///   （现 (110+128)/2=119=(101+18)，偏移 ±9 精确对称）；
    ...
    ///   现值：按钮 BottomMargin=7；配方 Gap=4；压力 Gap=6；SN Gap=4，关电流 Y=54；
    ...
    ///   204×160（行高 152→168），压力行(30)及以上不动，新增电流行(50,高16)+标签"电流："，
    ///   SN(70)/配方(95)/延时(118/136)/按钮(117)整体下移 16，间距全都不变；
    ///   false 时与本图逐像素一致。开关走 ShowCurrentRow 属性（MainForm 按 UsePowerMeter 装配一次），
    ///   行高/画布/命中一律走 GetEffectiveRowHeight()/GetEffectiveInnerHeight()，禁止手写 144/152。
    /// - 值框文字左内边距：ValueTextLeftPadding=6px（V1.52，文字不贴值框左边框，值框坐标不变）
    /// - 状态块配色见下方"状态块配色"；颜色值收敛在 PanelLayoutConfig，改代码生效
    ...
    public partial class WorkstationGridView : System.Windows.Forms.UserControl
    {
        ...
        /// <summary>
        /// 设置按钮字体（独立大字：按钮框 42×36，"设置"两字在正文字号下只占角落；
        /// 字号取配置 SetButtonFontSize（缺省 11），跟 zoom 等比缩放，与 RebuildFonts 同建同释放）。
        /// </summary>
        private Font _setButtonFont;
        /// <summary>
        /// 延时/烧屏时间值字体（比正文小半档：时间框 74 是全套最紧的槽，
        /// "00:00:00" 10pt 要 71px 会截断，9.5pt 约 63px 刚好装进 74-6=68px 文本区；
        /// 字号取配置 TimeValueFontSize，
        ...
        public bool ShowCurrentRow
        {
            ...
            set
            {
                ...
                {
                    UpdateCanvasSize();
                    // 内容高变了（面板 144→160），自适应 zoom 跟着重算。
                    UpdateAutoFit();
                }
                Invalidate();
            }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (clip.Right > ScaledX(_columns * _layout.PanelColumnWidth))
            {
                for (int row = startRow; row <= endRow; row++)
                {
                    // 按钮高取"面板内容高-1"（=143）：边框画在矩形下边界，不减 1 底边比面板内容底凸 1px；
        ...
        private void DrawPanel(Graphics g, GridItem item, int panelLeft, int panelTop)
        {
            ...
            // 时间框走小半档的时间字（74 框最紧，"00:00:00" 10pt 会截断、9.5pt 装得下；标签仍是正文 10pt）
