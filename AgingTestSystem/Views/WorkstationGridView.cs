...
namespace AgingTestSystem.Views
{
    ...
    ///
    /// 二、单个面板内容（222×205，坐标均为"相对面板左上角"；V1.58.19 只加锚定、位置与 V1.58.18 完全一致）：
    /// ┌──────────────────────────────────────────────┐
    /// │ NO.1（标题，左上角）            ┌────────────┐│
    /// │ ┌──────────┐  ┌──────────┐     │ 选中指示框  ││ ← 右上角 23×23
    ...
    /// │          └───────────┘                        │
    /// └──────────────────────────────────────────────┘
    /// 标注说明（V1.58.19 加锚定后坐标不变，括号内为锚定关系）：
    /// - 行1：上电/下电块(57,29,60,23) + 工作状态块(145,29,60,23) + 选中框(194,4,23,23)
    /// - 行2：真空压力值框(57,67,85,21；双端锚定+RightToLeftGap=3，右缘142距真空关左缘145留3px间隙、
    ///   下缘贴 SN 上缘 Gap=5) + 真空开/关块(145,67,60,21；下缘贴 SN 上缘、Gap=5)
    /// - 行3：SN 值框(57,93,148,21；下缘贴配方上缘、Gap=4)
    /// - 行4：配方值框(57,118,148,21；下缘贴设置按钮上缘、Gap=6)
    /// - 行5：延时开启值框(57,147,80,21；以设置按钮中心为基准、CenterOffsetY=-12) +
    ///   设置按钮(145,145,60,50；下缘距面板底 BottomMargin=10) + 延时到达值框(57,172,80,21；CenterOffsetY=13)
    /// - 标签列：真空压力(1,70)/SN:(1,96)/配方:(1,121)/延时开启(1,150)/延时到达(1,175)
    ///   （X=1 为右缘贴合压力框左缘推导 57-56=1；Y 以各自框中心为基准、VerticalCenterOffset=-1，均不变）
