...
namespace AgingTestSystem.Dialogs
{
    ...
    public partial class FanTestForm : UIForm
    {
        ...
        /// 首行置 _closed 拦后台 RunOnUi/AppendLog；定时器 Stop+Dispose 包 try。</summary>
        ...
        /// 关后不碰硬件（与通讯窗 TryConnectSilent 同因）。</summary>
        private void ConnectInternal()
        {
            ...
            RunOnUi(() =>
            {
                ...
                // 执行期再查：排队期间关窗则定时器已释放，Start 即炸，丢弃。
        ...
        /// 包 try/finally：旧版异常即卡死 _refreshBusy（以后永不刷新）；关窗直接丢弃。</summary>
        ...
        /// 关后不发硬件命令：关闭瞬间在途的启停若继续下发，等于"关了窗还在动设备"。</summary>
        ...
        /// 关窗竞态下已排队回调进到这里即拦（与通讯窗 SetConnected 同因）。</summary>
        ...
        /// <summary>
        /// 运行状态枚举 → 中文显示文本
        /// 与主窗体口径对齐：ProgramRunning 显示"程式运行中"
        ...
        /// 加 _closed + 句柄双查：旧版只查 IsDisposed，关闭瞬间后台命令仍能 BeginInvoke
        ...
        /// <summary>追加一行带时间戳的日志
        /// 旧版直调 txtLog.AppendText（调用方全在 UI 线程时没事，但关窗竞态下句柄已毁即炸）；
