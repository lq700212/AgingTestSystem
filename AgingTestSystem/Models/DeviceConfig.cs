...
namespace AgingTestSystem.Models
{
    ...
    public class DeviceConfig
    {
        ...
        /// <summary>
        /// 压力报警总开关（默认 true = 现状：阈值越限与真空建立超时正常报警，断电保护）。
        /// <para>做什么：false 时阈值越限与真空建立超时全不报，负压阀与载台电保持常开（只等时长到或人工停止）；DI 触点、通讯失联、自定义规则不受影响，照常报警。</para>
        /// <para>为什么是独立开关：报警方向 true/false 都有含义（高于报/低于报），再塞第三态会撕裂老配置；开关与方向正交，预置 A（常用·常开）默认关闭，B/C/D 默认开启。</para>
        /// <para>怎么改：跟项目（Policy.json 可按产品覆盖，机器缺省 true）；关了之后面板真空块恒绿（无阈值可判，按到位显示）。</para>
        /// </summary>
        public bool PressureAlarmEnabled { get; set; } = true;

        /// <summary>
        /// 报警全关·最高级（默认 false = 现状：各项报警按各自开关判定）。
        /// <para>做什么：true 时全部静默——阈值越限/真空建立超时/DI 触点/通讯失联/自定义规则/超温联停全不报，不断电、不标故障、不判结果（只等时长到或人工停止）。优先级高于压力报警总开关（L1 开着也被压住）。</para>
        /// <para>为什么独立一项：L1 只管真空类（DI/失联/规则照报）；空载调试/无管路点火要连 DI/失联一起压住，只能再加一级。四个预置默认全关，只走手动或自定义开启——量产默认必须报警，静默永远是显式 opt-in。</para>
        /// <para>怎么改：跟项目；风机断连策略是连接管理（非报警），不受影响仍按原策略走；完成表达式提前完成（非报警）也不受影响。</para>
        /// </summary>
        public bool MuteAllAlarms { get; set; } = false;
