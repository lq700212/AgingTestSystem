...
- 枚举与配置值的存储约定（**改动串口/配置相关必须先读此处**）：
 ...
 （到底值==理论最大＋横向条无＋2 秒稳定；回归无句柄跑不了 AutoScroll，不进回归，harness 即证据）。
  ...
  V1.92 下电左缘对齐值框列（双端锚定宽 49）、选中框 16→14、时间值单独 9pt（72 框最紧，"00:00:00" 10pt 会截断），
   ...
   V1.96 目检五处微调（设置按钮 46→42＋字 12→11pt 留 5px 缝、时间字 9→9.5pt 框 72→74、
   上电/真空块 18→20、配方框 16→18，面板 140→144，zoomY 微降字号不动）；
...
- **热更/数组/跨度三坑**：按 `TotalBarometers` 定长数组，热更改工位数必须走
  ...
  **先写后清**（清了状态写失败=软硬分叉，UI 弹框明示；完成/复位/急停/放弃恢复四路径同口径：
  判定侧只举旗不动状态，变迁落在 IO 成功之后，写失败保持原态记事件可重试，
  急停失败快照不删）；时钟类作差（重连节流/计时）负跨度钳零，
  防时钟拨慢饿死。
...
- **启动路径耗时操作禁上 UI 线程（V1.100 血泪）**：WMI 搜索/全量 72 台首轮采集可达分钟级，
  上 UI 即卡死全部 `BeginInvoke` 状态刷新（现场实锤：扫码枪 WMI 卡 UI 近 2 分钟，
  气压表/送风机状态等扫码枪那句 LOG 才更新）。三件套：耗时放线程池（只锁对象与状态，
  搜索本身不持锁）＋各设备启动并行（扫码枪先行、`Start` 先报连接事件后采首轮）＋
  串行排队（扫码枪等首轮）一律拆掉。回归锁"连接事件先于首批数据"（旧序反向必红）。
- **心跳/轮询日志只记边沿（V1.100）**：连上/断开各一行，中间过程静默；调试明细默认关闭
  （`ScannerDebugLog` 发货值曾误开，每 3 秒一条刷屏），开了也只在"端口快照变化"时记一行。
  复查先问"稳态每周期打是不是同一句话"，是就收成边沿。
...
- **递归防护数"真递归"**：平坦路过不消耗预算（规则解析器：括号分支+一元符分支才计数，
  ...
  （平坦长链求值左斜树同样栈溢出，节点总数另计预算，`MaxNodeCount`）。
...
- **Mock 不替装配层做使能决策**：显式注入优先于开关（如 MockFan＋禁用配置照连，
  开关只在 DeviceManager 装配传 null 时生效）；Mock 只拦 null 配置。
  加限制前先问"生产里这个组合到得了这里吗"，否则替身越俎打断正常用例。
...
- **用例假绿四查**：①反射探针先验"探针本身有效"（`Disposed` 是事件不是属性、`nativeBrush`
