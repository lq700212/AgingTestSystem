...
| PanelLayoutConfig | 默认布局基准坐标（V1.91 紧凑 204×128：按钮149,85,46,36/选中框183,5,16/第一行四件套同行67,4+125,4/标签列65/SN宽121/压力宽121右缘同界）、ResolveAnchors 幂等零漂移、高度+10 下链跟随+上链锁定+交接缝吸收（V1.77：SN/压力定高位现46/26）、宽度+10 右锚定组随动（含真空/下电跟选中框）、颜色解析钳位/回退、双实例解析零差异＋无文件口存在性锁（V1.91 删 PanelLayout.json 存取）、电流行几何（V1.77：缺省关/有效高144行高152/电流行74,44,121,16/SN62配方81按钮101/压力26不动/标签44/SN标签62/开关往返零漂移）、工作状态块删除（V1.88.16：真空块上第一行4当基准/下电跟随/删除三处无残留锁） |
| HomeLayoutConfig | 默认值、Save→Load 往返、范围约束、损坏文件回退默认、越界文件按Range钳（V1.88.17：5000高/负宽进不了主窗）、并单行（V1.88.23：HeaderHeight默认36/范围34~100/老双键文件读36/主窗单行字段锁/编辑器三输入框+顶栏量程；V1.88.28：顶栏锁死30存读归位/编辑器顶栏行边双删/下拉尺寸纯函数四锁） |
