...
| LicenseV183(V1.83 新增) | 机器指纹（本机可算64hex）、规范串稳定（同值同串/改值变串）、签名互逆（原样过/改项目拦/换钥匙拦）、授权文件落盘读回、持证四道关（全对放行/标题挂到期/机器不对+分量全不对拦/只换一块硬件漂移放行+警告/限定版跑新项目拦/通用版不限项目/点数超配拦/过期1天宽限放行/过期8天拦/功能超范围放行+警告）、改文件一个字验签拦、空指纹证阻断(V1.83.1 fail-closed)、标题后缀纯函数(V1.83.1 替换不堆叠，直调WithLicenseSuffix；V1.84 增挂后缀/空后缀/空标题)、试用双记（首跑放行/31天拦/时钟回拨不延长剩余天数，隔离目录+隔离注册表GUID子键+运行时现造测试密钥经反射缝，finally全复位）、授权窗只读模式导入禁用+机器码非空（构造不断言弹窗）；V1.84 补齐：点数字段非法=证损坏不误报超配；ProcessPolicyV170 补齐锁（V1.83 阈值/破空总闸/MES总闸/画面维度/口径报表挂节点+缺省副标题全显示）；V1.86 眼睛显隐16条（无参构造/眼睛是显示框子控件（非窗体叠放，规避层级坑）/贴右缘/垂直居中/默认掩码+隐藏态+悬停/点开三件 Shown+明文+悬停/收回三件/值不丢；PerformClick未Show空操作，直接调BtnEye_Click；自绘 EyeIcon 不走 emoji，仿 HJVision V4.4.3 经验） |
