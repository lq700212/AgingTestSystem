...
description: AgingTestSystem 项目专属的最终测试验证技能：一键完成"构建 → 真机冒烟测试 → 全量回归测试用例"。回归 harness 覆盖 PasswordHasher/UserManager 登录权限/配置归一化/IO 映射解析/配方存储/双日志器/面板布局锚定联动/工艺策略/项目档案热更删除/终结器释放/关窗竞态/MES映射上报/规则表达式/工艺策略窗/电流报表画面/软件激活(HJVision同源MD5三件套)等全部核心逻辑类（2225 断言）。当用户要求"跑测试、冒烟测试、回归验证、测一遍、发布前验证、改完代码验证一下"或修完 bug/加完功能需要验证时使用；新增测试用例也必须沉淀到本 skill 的 tests/TestRunner.cs 中。
---
...
## 三、测试覆盖范围（47 个模块，2225 断言）

| 模块 | 覆盖点 |
| --- | --- |
| PasswordHasher | PBKDF2 格式自描述、盐随机性、正反例、损坏串静默失败、中文/超长密码 |
| UserManager | 默认账号、登录边界（空值/Trim/大小写/角色错配）、AddAccount 唯一性与保底、RemoveAccount 保护、改密链路、HasPermission 权限矩阵、记住登录往返、Users.json 损坏回退重建、缺角色自动补齐、手改双管理员防呆；V1.64 起 dev 最高权限 J 组 34 条（种子+隐藏登录+注册保护不可用+删改矩阵+老文件自愈+手改保留 dev+第一业务管理员）；登录窗初始焦点 6 条（三角色账号非空＋光标进密码框，反射调 Load 不 Show）；V1.97 用户文件绝对路径化 3 条（隔离缝落盘+CWD干净+默认账号可用，EnterCleanDir 自动跟随缝） |
...
| PanelLayoutConfig | 默认布局基准坐标（V1.91 紧凑 204×128、V1.95 加高 204×140、V1.96 目检微调 204×144：按钮153,101,42,36/第一行四件套同行/上电74,4,45,20/真空123,4,54,20/标签列65/SN宽121/压力宽121右缘同界/延时74宽右缘距按钮5px/时间字9.5pt/设置字11pt；V1.92：下电74,4,49宽左缘对齐SN＋右缘贴真空4px/真空127,4,54/选中框185,6,14；V1.94：选中框181,4,18/真空123,4,54/下电74,4,45宽；V1.95：压力28隙6/电流48隙4/SN52隙4/配方77/延时98/烧屏116/交接缝9/行选按钮高139；V1.96：压力30/电流50/SN54/配方79,高18/延时102/烧屏120/交接缝仍9/行选按钮高143/内容1720×1368）、ResolveAnchors 幂等零漂移、高度+10 下链跟随+上链锁定+交接缝吸收（V1.77：SN/压力定高位现54/30）、宽度+10 右锚定组随动（下电左缘跟SN宽45）、颜色解析钳位/回退、双实例解析零差异＋无文件口存在性锁（V1.91 删 PanelLayout.json 存取）、电流行几何（V1.77：缺省关/有效高160行高168/电流行74,50,121,16/SN70配方95按钮117/压力30不动/标签50/SN标签70/开关往返零漂移）、工作状态块删除（V1.88.16：真空块上第一行4当基准/下电跟随/删除三处无残留锁）、标签居中绘制（V1.97：CenterRectOn+ComputeLabelRect 六行按行高以同行框中心为基准——压力/SN/配方/延时/烧屏/标题，开电流标签随电流框；行高实测下限+空参不炸）、公共参数负压tooltip（V1.98：与设置表同源双挂——写两边/-3 -7方向/配方回退/换行≤40/停留15秒；血泪：无托管组件窗禁new ToolTip(components)，走无容器+Disposed释放） |
| HomeLayoutConfig | 主页固定布局（V1.92 删 HomeLayout.json＋编辑器＋菜单入口＋设置表行：顶栏/状态栏30常量/右侧单参比例/类型与入口删除锁/设置窗单参构造；分隔条 IsSplitterFixed 锁死） |
| ModelRoundtrip | RecipeConfig/UserAccount JSON 往返（含特殊字符）、StationInfo/FanData Clone 深拷贝互不影响 |
| AgingSequencer | ShouldPowerOn(压力×延时双条件)、ShouldComplete(0=不限时长)、IsVacuumBuildFailed(到位即不失败) 边界族、IsPressureOutOfRange 双方向+恰等不越限(V1.62)、负时间语义锁、BuildStartWarningText 0时长/空SN 警告文案(V1.66)、IsFanOverTempShutdown 开关+上限+边界(V1.66)、IsVacuumBuildFailed零宽限=关闭4条(V1.104：启动/久等/负数/到位皆不失败，老立即失败语义作废) |
...
| MockDevices(V1.62) | 三 Mock 未连接约定/越界/副本隔离、气压两档区间千次采样、风机启停守卫与漂移界；V1.97 口径收敛 8 条（风机Connect(null)拒+注入优先于开关+启停读写正常、IO取反存物理读业务+批量+输入反值、电表释放走Disconnect） |
| StationCache(V1.62) | 往返全字段、覆盖语义、副本双向隔离、脏文件三态、非法编号过滤（反射重置静态缓存+隔离目录）；V1.97 脏读指纹 2 条（基线可读+手改文件即生效） |
| ModelDefaults(V1.62) | DeviceConfig 全构造默认值、风机枚举寄存器值、FanData/BarometerData Clone 全字段与数组深拷贝、LoginResult 工厂、角色值、快照与配方构造默认 |
| SettingsValidate(V1.62) | ValidateValue 全类型矩阵、TryParseUShort、范围表抽查+默认值落界、布尔键一致（V1.74：16 项含 UsePowerMeter）、连接键契约、CreateValueCell 全分发、分类/说明键对齐（构造真窗体不断言弹窗）、PersistChanges 统一路（V1.71：矛盾/MES 拦截、策略落盘+热回写、机器键落盘、空改动、null 不抛，运行目录隔离+备份还原）、三非模态弹窗关闭即释放（V1.72.13：走生产挂接反射调 ShowXxxPopup→OpenForms 找窗→Close→IsDisposed）、报表列/显示字典校验分支（V1.74）；V1.97 校验收紧 8 条（映射表空/ canonical 过+脏文本拦、停止位三档过+7拦、温度0~200过+越界拦）；V1.104真空超时(0关闭+档位过/负数拦/非整数与超限拦、不在数字键表、可输入下拉分发4条、策略分流保存3条) |
| ScannerParse(V1.62) | JoinPorts、ParseParity/ParseStopBits、与设置窗 NormalizeStopBits 跨文件 15 口径；V1.100 非阻塞+不刷屏 2 条（Start毫秒级返回+坏端口多Tick失败只一行） |
...
| HistoryCsv(V1.62) | CSV 解析边角、与 TestEventLogger 互逆 11 列（V1.76：SN/配方/结果结构化+报表格映射锁）、报表列设置按钮权限门与布局锁（V1.75：默认无权限/按钮五字/无重叠/不出右界/宽容五字/管理员有权限）；V1.97 多行拼合 7 条（IsQuoteUnbalanced平衡/未闭合/转义+详情换行拼合11列内容还原编号事件对） |
| UiPureHelpers(V1.62) | 批号去空格、配方查找(ignoreCase)+25h 不截断、工位温度读取(V1.63 数字框恒合法+回填钳制)、IP 合法、数字格钳制、网格命中/边界/四色、位值→通道、风机中文(V1.63 对齐主窗)、CH340 谓词/串口参数钳制(V1.63)、右侧宽度比例 ComputeRightPanelWidth(V1.65 比例＋V1.92 固定＋V1.93 收窄＋V1.94 到底：0.16 常量/护栏200~240/兜底 7 条，单参无文件；按钮=右侧-38/162不断字底线）、配方窗负压/显示模式框回填(V1.66)、反射 as-cast 跟随控件换型（V1.71：TextBox→UITextBox 两处）、两窗tooltip+破空显隐纯函数（V1.73）、配方管理窗6项tooltip全覆盖+换行锁（V1.86.5）、参数入口无权限提示3条（V1.88：有权限放行/无权限含权限不够/指明去用户权限）、时间名统一（V1.88.12：两窗tooltip锁"对应工位面板延时时间/烧屏时间"+配方窗标签"延时时间：/烧屏时间："）、真空块三色（V1.88.12：ApplyData反射阀关灰/阀开到位绿/阀开未到位红+红绿异色+下电逻辑不变）、选中框常显签名锁（V1.88.12：DrawPanel去anySelected+旧调度已删+IsAnySelected门控保留）、单击点选（V1.88.14：长按整套删除IsAnySelected/ClearAllSelection/_longPressTimer/Tick+ToggleSelect无门槛翻转反射+按宽自适应ComputeFitZoom宽口径/zoom并进Scaled/字体下限6pt/AutoFit默认开）、延时0填0阀电同开tooltip锁（V1.88.14：三窗"同时开"）、双向精确铺满（V1.88.17：ComputeFitZoomBoth独立/非法回1,1/MinMax0.15/4/ScaledX/ScaledY/矩形双向/挂载双轴+铺满±1px/选中框正方形+命中同源/保Panel1宽640七条）、小屏字号（V1.88.21：MinFontSize6→4/1280真实zoom跟随缩小+标签时间装槽两条）、双模式大字（V1.88.22：ComputeFitZoom单轴捞回/默认FitWidth/双模式挂载+字号等比9条）、删滚动单路（V1.88.24：枚举/FitMode/单轴/MinMax/拖拽Tick/校正删除锁+挂载精确铺满/窄边4pt/面板加粗+顶栏分区字段/SetFontSizePt三锁）、顶栏列宽高度（V1.88.26：默认30/范围28/回退老文件30/量程28/钳-5→28/纯函数常规封顶非法三锁；V1.88.27：权限区Panel类型锁，与项目区同构Dock居中；V1.88.28：行全选竖排九锁——倍率缺省2/9→18加粗/单字格/间隙/整块居中/块底/零字/下限/重建18pt/度量缓存；V1.88.29：标题12/设置12缺省/三绿深绿值/设置字体四锁/标题槽位/重建12pt；V1.89：全选列缺省48/内容宽1720/挂载zoomX期望同步/去标题收益锁（976×845→zoomY=845/1638/正文≈4.64pt)/窗口三按钮穿透判定与明暗配色/版本弹窗跟水印常量；V1.90：工作站紧凑（内容1224/挂载zoomY=600/1224/正文≈4.65pt/去标题收益窄边横向≈5.67pt/第一行同行无交叠/短标签两字装槽/时间串装72框/标题11pt装61槽/行选重建20pt/按钮框50×36）；V1.91：长标签恢复（四字装65槽/值列X74宽121/延时右缘距按钮3px/按钮46×36设置完整显示/删PanelLayout.json存取改纯代码缺省）；V1.92：下电左缘对齐SN＋右缘贴真空4px/标题68槽/上电装49块/真空装54块/时间值9pt（00:00:00与88:88:88装66区/9pt加粗/下限4pt/重建9pt）；V1.94：选中框18对勾保留（Y4与真空同顶/真空X123/上电45各留4px/同行选中Y4/扁拉伸196,201）；V1.95：面板加高（内容1720×1332/挂载zoomY=600/1332/去标题845/1332/800×600窄边翻纵向正文≈4.50pt/行缝[142,148)测144/底边141/行选按钮高139/状态栏不动）；V1.96：目检微调（按钮42×36设置11pt完整显示/时间9.5pt装74框68区/上电45×20真空54×20/配方121×18/延时右缘距按钮5px/内容1720×1368/挂载600/1368去标题845/1368/窄边≈4.39pt/行缝[146,152)测149底边143行选高143/高度+10按144基准：按钮111配方89缝19延时112/130）；V1.101 通讯窗三页统一 13 条（未连接/关后写返回false+负压灯回滚双向+预留DO回滚+DO/DI存在+DI快照点亮+提示入口存在与关后静默+自动同步入口与后台收尾） |
...
| PolicyPresetV185(V1.85) | 预置4个试用顺序A/B/C/D、管辖14开关全是PolicyKeys成员+中文名齐、每预置完整14项、标题场景换挡文案非空、值全可解析+枚举值全在下拉选项、套用探测往返、缺省=自定义/改一项即自定义、未知与空参四不抛、取值副本隔离、四预置不泄压（无阀）不跳真空、B/C联停关D开+上限0拦60过+B零门槛过、存盘口径三态、UI预置行4件+下拉5项+无参回显自定义禁用/B配置回显选中B/管理员可用/场景说明（反射读Items/SelectedIndex，禁as原生类型见坑41）、下拉列表拉宽+悬停全文+关窗释放；V1.105 预置D客户常开20条（4个顺序A/B/C/D＋往返探测＋D与A只差失压＋数值超时0成员解析校验副本合流＋ABC数值空＋D零门槛＋D整包分流＋下拉5项＋自定义顺延4＋D回显选中＋无配方延时0集成2条；NumericValues只收PolicyKeys数值）；V1.106 顺序轮转A=常用/B=标准/C=宽松/D=严格23条（A标题场景常用常开＋A回显＋行标题A/B/C/D＋A与C十四项全同＋A超时0成员合流＋BCD数值空＋A零门槛＋A整包分流＋C超时0探测为A＋下拉即预览隔离不脏D预览＋BuildPreviewConfig＋关窗IsPreviewPending四态＋文案两路；带数值项的预置数值算身份）；2.0.0 自定义槽与导入导出37条（标题说明悬停无括号＋槽是跟项目文件＋初始=A快照自由文本跟种子＋二次Ensure不覆盖＋改槽A不动＋脏key过滤＋导出装包口径＋导入往返三路拦停＋导入导出按钮末项文案使能＋只读可导不可入＋三参预览判定＋槽隔离备份恢复）＋动态说明7条（默认文案悬停无括号＋初始空null改后四态pristine＋未改动回显默认改后切差异）＋2.0.0右栏跟随预览10条（选中节点右栏=真实/切A画布右栏同步0左右同源/真实不动/不标脏/预览标识＋状态条明示/切回B恢复清除）；2.0.0 压力报警总开关13条（四参纯函数6：两方向开关/常压/旧口一致＋A关BCD开＋A/C除开关外全同＋C超时0报警开仍C/加报警关才A＋整包分流走A＋设置表布尔19项（含全关）＋报警开关挂节点首项＋端到端报警关3条＋布尔中文显示2条）＋停止复位即时刷屏6条（停止/复位当场广播＋回空闲清结果＋输出OFF，E15b/c现场复位延时压力）＋防挂起收口4条（helper存在性＋置脏照切行为＋显示关闭开启＋存盘仍true-false，5处裸调全迁入）＋2.0.0旧槽回填5条（缺新key按A补报警关＋落盘＋回填后与A一致＋不动已改项＋改过仍自定义）＋生效缺省7条（A管辖回A/自由文本null＋叠加缺key/数值/显式优先/自由文本不管/文件压内存策略优先）＋L2全关19条（纯函数2＋四预置全关＋节点副标题2＋classify越限超时DI空数据对照5＋live失联3规则2越限3）＋定位点名8条（人人有节点＋3处点名＋null＋比对提示3） |
...
| MainFormIconV107(V1.107) | 任务栏窗口图标恒等于exe文件图标6条（反射调MainForm.LoadTaskbarIcon静态helper，不构造主窗：helper存在＋产品exe返回非空＋与文件关联图标同尺寸＋不存在路径回null不抛＋空串与缺省同路回退自身＋缺省与显式传自身一致） |
...
51. **前后差值断言的 marker 必须互斥，失败行文案含成功子串即双计假红**
    （V1.97：S18 成功 marker 用"全部停止"，失败行"全部停止下发失败"同样命中，
    差值恒 +1 精准红。修法：成功 marker 改"关闭所有阀与载台电"（失败行没有）。
    **教训：定 marker 先拿失败行全文做子串检查；事件名/详情各取互斥词，
    别用"大词套小词"。**）
52. **完成变迁必须收归一处，判定侧只举旗不动状态**
    （V1.97：采集侧 ShouldComplete 即置 testing=false，完成写失败已无法回头="假完成"。
    修法：举旗与变迁分离，`CompleteDeviceInternal` 返回 bool、内部先写后清，
    失败调用方直接 return 不碰广播。**教训：凡"先改状态、后做 IO"的两段式，
    失败路径永远有一段回不去——状态变迁只许落在 IO 成功之后。**）
53. **Mock 不替装配层做使能决策，显式注入优先于开关**
    （V1.97：MockFan 加 FanEnabled 拒绝后，注入 Mock＋禁用配置的扩展用例全红——
    生产装配禁用本就传 null 控制器，开关只在装配层生效。修法回退守卫，
    Mock 只拦 null 配置。**教训：测试替身保持 dumb，策略判断归接线层；
    加限制前先问"生产里这个组合到得了这里吗"。**）
54. **运行时文件绝对路径化后，测试隔离必须跟缝走，CWD 切换即失效**
    （V1.97：UserManager 改 BaseDirectory 后旧用例全读 run 目录文件。
...
56. **用例里禁裸调一切会弹模态框的方法，违者无人值守必卡死**
    （2.0.0：工艺策略窗布尔中文显示用例新增切节点块，撞上前一步"改选同步换"
    留下的脏态，`SelectNode` 弹"是/否/取消"把测试线程永远卡住，现场等人点鼠标；
    前两次全绿是人手点的，不是真绿）。
    修法：测试侧收 `SelectNodeNoHang` 唯一入口（先清脏再调，只读导航；
    脏提示本身是生产行为，走真窗手工测，不许为测试阉割），5 处裸调全迁入，
