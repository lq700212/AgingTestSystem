...
$Map = @(
    ...
    @{ Pat = @("*CommonParameterForm*");            Mods = @("UiStyleV172_1") },
    @{ Pat = @("*HistoryRecordForm*");              Mods = @("HistoryCsv", "UiStyleV172_1") },
    @{ Pat = @("*SettingsForm*");                   Mods = @("SettingsForm.Normalize", "SettingsValidate", "PolicyV167", "MesV168", "RuleExprV169", "ScannerParse", "DeviceConfig.ParseFanIpCandidates", "IoOutputChannelRemap") },
    @{ Pat = @("*BatchRecipeForm*", "*StationSettingsForm*", "*RecipeManagerForm*"); Mods = @("UiPureHelpers", "LegacyRecipeGuard", "RecipeStorage") },
    @{ Pat = @("*IdBindingForm*", "*InputLotForm*"); Mods = @("UiStyleV172_1", "UiPureHelpers") },
    @{ Pat = @("*ChangePasswordForm*", "*LoginForm*", "*UserManagementForm*"); Mods = @("UserManager") },
    @{ Pat = @("*UnloadJudgeForm*");                Mods = @("PolicyV167", "DeviceManagerPolicy") },
    @{ Pat = @("*ProjectSwitchForm*");              Mods = @("PolicyV167") },
    @{ Pat = @("*HomeLayoutEditorForm*");           Mods = @("HomeLayoutConfig") },
    @{ Pat = @("*CommunicationTestForm*", "*FanTestForm*"); Mods = @("ScannerParse", "ModbusConvert", "FanParse") },
    @{ Pat = @("*FlowCockpitForm*", "*FlowGraph*"); Mods = @("FlowCockpitV170", "PolicyV167") },
    @{ Pat = @("*MainForm*");                       Mods = @("UiPureHelpers", "UiStyleV172_1", "ThemeManager", "SettingsValidate") },
    ...
    @{ Pat = @("*ProjectPolicyStore*", "*PolicyEnums*"); Mods = @("PolicyV167", "SettingsValidate", "DeviceManagerPolicy", "FlowCockpitV170") },
    ...
    @{ Pat = @("*RecipeStorage*", "*RecipeAutoCompleteProvider*"); Mods = @("RecipeStorage", "UiPureHelpers") },
    ...
    # --- Models ---
    @{ Pat = @("*DeviceConfig*");                   Mods = @("ModelDefaults", "SettingsValidate", "PolicyV167", "MesV168", "FlowCockpitV170", "DeviceConfig.ParseFanIpCandidates", "DeviceManagerIntegration") },
    ...
    @{ Pat = @("*HomeLayoutConfig*");               Mods = @("HomeLayoutConfig") },
