# ============================================================================
#  obfuscated_release.ps1 - One-click obfuscated release pipeline (V1.88.10 起):
#      前置检查 -> Release 构建(临时翻混淆标记) -> Obfuscar 混淆 ->
#      组包 -> 验收(行为+对账+冒烟) -> 还原标记
#
#  Usage (from anywhere):
...
#
#  Outputs (all under <repo>\release\<Label>-obf\, gitignored, never committed):
#      包\        发给客户的全部文件（混淆 exe + 依赖 dll + exe.config + 部署说明.txt；
#                 无 pdb/源码/旧数据，拷到工控机即用）
#      归档\      内部留底（Mapping.txt + 全包 MD5 + pdb + 本次配置 + git 版本；
#                 客户发回堆栈靠"水印版本→这套归档"反解）
#
#  Acceptance (any failure aborts with non-zero, package kept for inspection):
#      A. behavior: 公开 API 行为 12 项（水印版本+混淆标记/日志落盘/反射点/JSON/规则/激活）
#      B. dump+diff: Models 元数据 1:1 对账（未混淆 Release 是标准答案）
#      C. smoke: 混淆 exe 真启动 22s 存活 + AppLog 首行水印含版本与混淆版 + 无 Crash 文件
#
...
$relRoot = Join-Path $RepoRoot ("release\" + $ReleaseLabel + "-obf")
...
try {
    & $msbuild (Join-Path $RepoRoot "AgingTestSystem\AgingTestSystem.csproj") `
        /p:Configuration=Release /p:Platform=AnyCPU /t:Build /nologo /v:m
    ...
    # 包里：混淆 exe + 未混淆依赖（第三方 dll 不混淆，只混淆自己的 exe）。
...
4. 出问题时把 Logs 文件夹整个发回（AppLog 定版本、Crash_*.log 看堆栈），
   外加故障台号与现象截图。
...
"@
    ...
    # 归档：Mapping + 全包 MD5 + pdb + 本次配置 + git 版本（客户堆栈反解全靠它）。
    ...
    [IO.File]::WriteAllText((Join-Path $arcDir "版本.txt"),
        ("ReleaseLabel=" + $ReleaseLabel + "`r`nIsObfuscatedBuild=true`r`n打包时间=" `
    ...
    try { & ".\Accept.exe" behavior $accDir $ReleaseLabel "1"; $cBeh = $LASTEXITCODE }
    ...
    if ($p.HasExited) { Fail ("[ACCEPT FAIL] 混淆包启动" + [int]($waited / 1000) + "s内退出，码=" + $p.ExitCode) 2 }
    ...
    if (-not ($logText.Contains("[启动]") -and $logText.Contains($ReleaseLabel) -and $logText.Contains("混淆版"))) {
        Fail "[ACCEPT FAIL] AppLog 首行水印不对（缺[启动]/版本/混淆版标记）" 2
    }
    ...
    # ---------- 还原混淆标记（必须执行：日常 Debug 包不能被标成混淆版） ----------
