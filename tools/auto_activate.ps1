<#
.SYNOPSIS
  烧屏测试控制中心工控机一键激活（与 HJVision 同源同口径，与 tools/auto_activate.py 同逻辑）。
.DESCRIPTION
  把原来四步手工活（一：授权窗抄设备ID/设备码；二：回办公室用《获取激活码》工具算码；
...
function Find-Slot([string]$stored, [string]$cpuId) {
  # 找计数格 0..839（与主窗 HashTimer_Tick 的 for 循环一致），找不到返回 -1
...
# ---------------- 主流程 ----------------
Write-Host "============================================================"
Write-Host "烧屏测试控制中心 一键激活（与软件内【关于→软件授权】同口径）"
Write-Host "============================================================"
...
Write-Host "------------------------------------------------------------"
Write-Host "[成功] 已激活（$statusText）。请重启烧屏测试控制中心，【关于→软件授权】应显示“$statusText”。"
