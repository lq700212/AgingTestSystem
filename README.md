...
>
> **72 站自适应（V1.88.14，列数保持 8×9 不动）**：中间工位区多宽、面板就自动缩放到多宽，
> 站尽可能大、无右侧空白；高度超出部分上下滑动看（窗口最大化/拉宽跟随缩放，
> 字小到 6pt 不再小；要回原来固定大尺寸，尽管把网格 `AutoFit` 关掉，默认开）。
