...
| `Views/ProcessPolicyForm.cs` / `Views/PolicyGraph.cs` | 工艺策略窗（V1.70 建图为"流程驾驶舱"，V1.73 改名：固定拓扑画布，节点显示真实配置+实时台数，点节点改配置走同一条保存路；滚轮缩放/中键平移/节点拖拽；预置 A 常用/B 标准/C 宽松/D 严格一键套用，下拉切档左侧即时预览只看不存，关窗时预览未套用弹 SunnyUI 确认框；入口=参数设置下拉） |
