...
| `CommunicationTestForm` | 【V1.20】通讯测试（技术员及以上） | IO 耦合器（GX-CL140，Modbus TCP）DO 输出通道手动测试窗体：两个页签分别测 72 路负压阀输出（Y000~Y107，寄存器 0x2000~0x2004）与 72 路载台上电输出（Y110~Y217，寄存器 0x2004~0x2008），每页 9×8 圆形灯按钮点击 toggle ON/OFF（读-改-写不覆盖共享的 0x2004 字节），底部 连接测试/全部关闭/读取状态/一键遍历/关闭窗口 五个按钮 + 日志框；复用生产工程 NModbus 独立连接，不影响采集线程；复用 App.config 备用通道映射（IoBackupChannelMappingEnabled / IoBackupChannelMappings，V1.20）。【V1.21】整体改用 SunnyUI 控件重构（UIForm 标题栏 + UITabControl/UIPage 页签 + UIPanel 面板 + UIButton 按钮 + UITextBox 日志 + UILedBulb 连接状态灯，顶部状态条实时显示已连接/未连接）；点击被映射到备用通道的通道时，弹出**非模态悬浮提示窗**（不阻塞流程、不抢焦点、可保持打开继续操作）告知"该通道已做备用通道映射、实际输出通道是哪个寄存器第几路"并在日志追加映射记录。【V1.22】新增"一键遍历"（紫）按钮：对当前页签测试做**通断跑马灯**检测——每 500ms 只点亮一路通道、其余全灭，72 路循环往复，用于快速检查每路 DO 接线通断；再次点击（变红"停止遍历"）立即停止并全部关闭；负压开关测试 / 载台上电测试 两个页签都支持，运行中切换页签自动停止。【V1.22 补强】①遍历单拍在**后台线程**执行（写寄存器 + 读回真实状态后切回 UI 线程刷新按钮），界面不卡顿，且按钮状态**实时反映真实通断**（接线异常一目了然）；②窗体打开即**后台自动连接**（失败只在日志提示，重连定时器每 3s 后台静默重试，成功自动恢复）；③**心跳机制**：已连接时每 1s 后台读 0x2000 探测存活，中途断连立即弹窗提醒并自动进入后台重连；④所有 Modbus 读写共用锁串行化，避免多线程并发操作。【V1.22 映射完善】备用目标寄存器 0x2009 写入改为**读-改-写**（`WriteBackupRegister` + `ComputeRemapTargetMask`，各测试只动自己映射目标位、互不覆盖，对齐主项目逐通道 RMW）；一键遍历点亮被映射通道时同样弹窗告知实际输出通道（按钮仍显示旧通道名）。【V1.22 提示体验】映射提示窗改为**非模态悬浮窗**（`RemapNoticeForm`）：不阻塞代码流程、不强制占用焦点（`WS_EX_NOACTIVATE`），用户可保持弹窗打开继续点击其他窗体；复用单实例只更新文本不重复弹窗 | — |
